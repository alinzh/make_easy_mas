...
## Официальный сервер mcp-server-fetch

Один из самых полезных серверов из официального репозитория Model Context Protocol - это `mcp-server-fetch`. Он взят из [официального репозитория MCP серверов](https://github.com/modelcontextprotocol/servers/tree/main/src/fetch) и предоставляет языковой модели возможность загружать веб-страницы.
...
Если вы предпочитаете npm, можно установить глобально:
...
Попробуйте запустить пример из `example.py`. Обратите внимание, как агент автоматически определяет необходимость использовать инструмент `fetch`, вызывает его с правильными параметрами и обрабатывает результат. Попробуйте изменить URL на другую документацию или статью и посмотрите, как агент справится с разным контентом.

Затем поищите в репозиториях другие интересные серверы и попробуйте подключить их к своему агенту. Возможно, вы найдете что-то полезное для ваших задач.
