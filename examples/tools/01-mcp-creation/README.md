...
В первой части мастер-класса мы разобрали три ключевых этапа работы с инструментами:

1. **Определение инструмента** - описание в виде JSON-схемы
2. **Вызов инструмента** - structured output от модели и его интерпретация
3. **Возврат результата** - передача данных обратно модели

Мы также увидели основные проблемы такого подхода:
- Фрагментация форматов между разными провайдерами
- Отсутствие стандарта для переиспользования инструментов
- Необходимость вручную управлять регистрацией и обнаружением
- Проблемы с безопасностью и валидацией

**MCP решает все эти проблемы**, предоставляя единый стандартизированный протокол. По сути, MCP использует те же три этапа, что мы разобрали, но добавляет:
- Унифицированный формат обмена сообщениями (JSON-RPC 2.0)
- Автоматическое обнаружение возможностей (`tools/list`, `resources/list`)
- Встроенную валидацию через JSON Schema
- Архитектуру клиент-сервер для изоляции и масштабируемости
